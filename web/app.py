...
def totem_checkin_confirmar():
    ...
    # Cria workflow automático (4 tarefas)
...
def paciente_app(token):
    ...
    etapa_index = ['recepcao', 'medico', 'farmacia', 'enfermagem'].index(etapa_atual) if etapa_atual in ['recepcao', 'medico', 'farmacia', 'enfermagem'] else 0
...
def api_pacientes():
    """API: Lista de pacientes em JSON"""
    todas_tarefas = tarefas._carregar_tarefas()
    return jsonify(todas_tarefas)


@app.route('/api/pacientes/<id>')
def api_paciente_detalhes(id):
...
def realizar_acao_tarefa(id):
    ...
    if acao == 'encaminhar_medico':
        # Recepção -> Médico
        # Tenta extrair especialidade da descrição
        ...
        flash('Paciente cadastrado e encaminhado para o médico.', 'success')
